...
function clickHere() {     
let text=document.getElementById("n").value;   //preiau textul pe care l-am adaugat in labelul de pe pagina HTML (care trebuie prelucrat)
...
let keys = Object.keys(vector).sort().reverse().sort((a, b) => {
...
let treeData = constructTree(sorted, 0, keys.length, "O", "A", 0)
stylee(treeData);
}

function constructTree(sorted, l, r, flag, name, level) {
    let node = {};
    ...
    let sum = 0;
    for (let i = l; i < r; i++) {
        sum = sum + sorted[keys[i]];
    }

    let suma = 0;
    let position;
    for (let i = l; i < r; i++) {
        if (suma + sorted[keys[i]] < sum/2) {
            suma = suma + sorted[keys[i]];
        } else {
            if (sum/2 - suma < suma + sorted[keys[i]] - sum/2) {
                position = i;
                break;
            }
            position = i + 1;
    ...
    node["children"] =
        [constructTree(sorted, l, position, "L", node["name"], level + 1),
        constructTree(sorted, position, r, "R", node["name"], level + 1)];
...
//De aici in jos am preluat codul din exemplul dat la laborator 
//  ||

function stylee(treeData) {
